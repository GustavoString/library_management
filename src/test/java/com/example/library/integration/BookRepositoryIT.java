...
class BookRepositoryIT extends AbstractIntegrationTest {
    ...
    class FilterTests {
        ...
        // =========================================================
        // === Person 3: genre / author / keyword search tests   ===
        // =========================================================
        ...
        void shouldFindByGenre() {
            // Arrange
            createBook("978-F1", "Dune", "Frank Herbert", 2, Genre.FICTION);
            createBook("978-F2", "1984", "George Orwell", 1, Genre.FICTION);
            createBook("978-S1", "A Brief History of Time", "Stephen Hawking", 3, Genre.SCIENCE);

            // Act
            List<Book> fictionBooks = bookRepository.findByGenre(Genre.FICTION);

            // Assert
            assertThat(fictionBooks).hasSize(2);
            assertThat(fictionBooks).allMatch(b -> b.getGenre() == Genre.FICTION);
        ...
        void shouldSearchByAuthorKeyword() {
            // Arrange — keyword "george" must match in the author OR the title field
            createBook("978-K1", "1984", "George Orwell", 1, Genre.FICTION);
            createBook("978-K2", "Curious George", "H. A. Rey", 1, Genre.CHILDREN);
            createBook("978-K3", "Pride and Prejudice", "Jane Austen", 1, Genre.FICTION);

            // Act
            List<Book> results = bookRepository.searchBooks("george");

            // Assert
            assertThat(results).hasSize(2);
            assertThat(results).extracting(Book::getTitle)
                    .containsExactlyInAnyOrder("1984", "Curious George");
        }
